<!-- resources/views/admin/surat-keluar/sku.blade.php -->
...
<td>: {{ $surat->pengajuan->data_pengajuan['nama_usaha'] ?? '-' }}</td>
</tr>

<tr>
<td class="label">Jenis Usaha</td>
<td>: {{ $surat->pengajuan->data_pengajuan['jenis_usaha'] ?? '-' }}</td>
</tr>

<tr>
<td class="label">Alamat Usaha</td>
<td>: {{ $surat->pengajuan->data_pengajuan['alamat_usaha'] ?? '-' }}</td>
</tr>

<tr>
<td class="label">Tahun Berdiri</td>
<td>: {{ $surat->pengajuan->data_pengajuan['tahun_berdiri'] ?? '-' }}</td>
</tr>

</table>

<p>
Benar memiliki usaha aktif di wilayah Desa Rambatan Wetan
dan sampai saat ini masih berjalan sebagaimana mestinya.
</p>
...
<p>
Demikian surat ini dibuat dengan sebenarnya agar dapat
...
<img
    src="{{ public_path('storage/'.$surat->pejabat->tanda_tangan) }}"
>
